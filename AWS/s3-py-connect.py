#pip install awscli
#aws configure
#install boto3 in pycharm
"""
import boto3
s3=boto3.client('s3')
s3.upload_file('s3-transfer.txt','92-s3-raj-bucket','s3-transfer.txt')
"""

# AWS Read CSV file data from S3 via Lambda function and insert into MySQL
#choose rds service and aws aurora through which u can store data in mysql
#s3-bucket name->bucket properties->events->all object create event->select lambda func
"""
import boto3
import pymysql
